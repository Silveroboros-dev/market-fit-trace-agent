...
|---|---|---|---|---|---|---|
| Live market ID namespace mismatch | `fit.py` has hard-coded fixture market IDs such as `pm_hormuz_traffic_normal_end_june_2026`, `pm_us_iran_permanent_peace_by`, `polymarket_*`; PolyData currently returns live numeric IDs such as `2155023`. A pattern branch can recommend a fixture ID that is absent from the live candidate set. | Fixture goldens protect static IDs: `eval_005`, `eval_006`, `eval_007`, `eval_010`; v2/v3 candidate Iran cases protect fixture IDs only. | No strict case checks that `recommended_market_id` must belong to the live retrieved candidate set. | "US and Iran will reopen the Strait of Hormuz by June 30, 2026" with PolyData returning numeric Hormuz markets. Verify whether the policy recommends a retrieved numeric market or a stale fixture ID. | Live retrieval candidate first; freeze the retrieval packet before turning into a strict golden. | P0 |
